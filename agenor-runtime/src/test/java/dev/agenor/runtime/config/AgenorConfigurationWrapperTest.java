...
/**
 * Unit tests for AgenorConfigurationWrapper
 */
class AgenorConfigurationWrapperTest {
    ...
    void shouldCreateWrapperWithNonNullConfiguration() {
        ...
        AgenorConfigurationWrapper wrapper = new AgenorConfigurationWrapper(config);
    ...
    void shouldCreateWrapperWithNullConfigurationUsingDefaults() {
        AgenorConfigurationWrapper wrapper = new AgenorConfigurationWrapper(null);

        assertThat(wrapper).isNotNull();
        assertThat(wrapper.getConfiguration()).isNotNull();
        assertThat(wrapper.getConfiguration().runtime().name()).isEqualTo("agenor-runtime");
    }
    ...
    void shouldDeserializeFromYamlCorrectly() throws Exception {
        ...
        AgenorConfigurationWrapper wrapper = mapper.readValue(
            yamlContent,
            AgenorConfigurationWrapper.class
        );
    ...
    @Test
    @DisplayName("Should use defaults when agenor field is missing")
    void shouldUseDefaultsWhenAgenorFieldIsMissing() throws Exception {
        String jsonContent = "{}";
    ...
    void shouldPreserveCompleteConfigurationThroughWrapper() {
        AgenorConfiguration original = new AgenorConfiguration(
        ...
        AgenorConfigurationWrapper wrapper = new AgenorConfigurationWrapper(original);
        AgenorConfiguration retrieved = wrapper.getConfiguration();

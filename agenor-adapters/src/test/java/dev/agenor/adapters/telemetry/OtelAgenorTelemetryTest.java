...
/**
 * Verifies {@link OtelAgenorTelemetry} emits correctly named spans with the right
 * attributes and status codes, using {@link InMemorySpanExporter} (no external collector).
 */
@DisplayName("OtelAgenorTelemetry")
class OtelAgenorTelemetryTest {
    ...
    void setUp() {
        ...
        telemetry = new OtelAgenorTelemetry(sdk);
    }
    ...
    void parentChild_contextPropagation() {
        // Use the same tracer name as OtelAgenorTelemetry.INSTRUMENTATION_NAME
        io.opentelemetry.api.trace.Tracer tracer = sdk.getTracer(OtelAgenorTelemetry.INSTRUMENTATION_NAME);

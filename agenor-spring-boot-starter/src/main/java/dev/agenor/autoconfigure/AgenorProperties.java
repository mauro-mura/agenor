 ...
 *
 * <p>The starter intentionally bypasses the {@code agenor.yml} file discovery performed
 ...
 * from a {@code agenor.yml} file, call
 * {@code AgenorRuntime.builder().fromClasspathConfig("agenor.yml")} manually and declare
 * your own {@code AgenorRuntime} bean (which will suppress this autoconfiguration via
 ...
 */
@ConfigurationProperties(prefix = "agenor")
public record AgenorProperties(
    ...
    public record Runtime(
            @DefaultValue("agenor-runtime") String name,
     ...
     * {@link AgenorConfiguration.AgentsConfig} by the core constructor.
     ...
     *                       with native {@code agenor.yml} format
     */
    ...
    public record Messaging(
         ...
         * @param consumerGroupPrefix     prefix for stream keys and consumer groups; default {@code agenor}
        ...
        public record Redis(
                ...
                @DefaultValue("agenor") String consumerGroupPrefix,
     ...
     *
     * <p>Only effective when {@code agenor-adapters} is on the classpath and OTel
     ...
     * @param serviceName {@code service.name} resource attribute; default {@code agenor}
     * @since 0.19.0
     */
    public record Telemetry(
            ...
            String endpoint,
            @DefaultValue("agenor")  String serviceName
    ) {}

    // ── Conversion ────────────────────────────────────────────────────────────

    /**
     * Converts to {@link AgenorConfiguration} for use with
     * {@link AgenorRuntime.Builder#withConfiguration}.
     *
     * <p>{@code agents.base-package} and {@code agents.scan-packages} are passed
     * separately; the {@link AgenorConfiguration.AgentsConfig} constructor merges
     ...
     * performed by {@link AgenorRuntime.Builder#withConfiguration}
     * when the runtime bean is created, not here.
     */
    public AgenorConfiguration toAgenorConfiguration() {
        ...
        return new AgenorConfiguration(
                ...
                new AgenorConfiguration.MessagingConfig(
                        messaging().provider(),
                        buildMessagingProperties()
                ),
                new AgenorConfiguration.DirectoryConfig(
                        directory().provider(),
                        buildDirectoryProperties()
                ),
                new AgenorConfiguration.SchedulerConfig(
                        scheduler().provider(),

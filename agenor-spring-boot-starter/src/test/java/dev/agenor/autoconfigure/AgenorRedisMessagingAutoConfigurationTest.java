...
/**
 * Tests for the Redis messaging conditional beans in {@link AgenorAutoConfiguration}.
 *
 ...
 */
class AgenorRedisMessagingAutoConfigurationTest {
    ...
    void redisSubRecordBindsTypedFields() {
        runner
            ...
            .run(ctx -> {
                var redis = ctx.getBean(AgenorProperties.class).messaging().redis();
    ...
    // auto-wiring (redisMessageDispatcher → AgenorRuntime) runs as normal.
    // -------------------------------------------------------------------------
    ...
    @Test
    void AgenorRuntimeIsWiredWithRedisDispatcherWhenLettucePresent() {
        RedisMessageDispatcher mockDispatcher = mock(RedisMessageDispatcher.class);
        RedisMessagingFactory  mockFactory    = mockFactory(mockDispatcher);
        ...
        runner
            ...
            .run(ctx -> {
                ...
                assertThat(runtime.getMessageDispatcher())
                        .as("AgenorRuntime must use the Redis dispatcher, not the in-memory default")

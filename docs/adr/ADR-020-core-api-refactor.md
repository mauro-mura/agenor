...
| `DirectMessenger` | `sendTo(recipientAgentId, msg)` |
...
`MessageDispatcher.sendTo(agentId, msg)` uses `AgentResolver.resolveEndpoint` to translate the
agent ID to a `TransportEndpoint`, then dispatches via `MessageTransport`. For `local` transport,
the in-memory path short-circuits without going through a transport object.

### Routing via `AgentResolver`

```
Agent → MessageDispatcher.sendTo(agentId, msg)
     → AgentResolver.resolveEndpoint(agentId) → AgentEndpoint
...
| `MessageService.send(Message)` | `publish(topic, msg)` or `sendTo(agentId, msg)` |
